...
export function BlogDetailClient({ post, relatedPosts }: BlogDetailClientProps) {
    ...
    return (
        <>
            ...
            <div className="fixed top-0 left-0 w-full z-[100] h-1 bg-slate-100">
                <div
                        ...
                        <aside className="lg:col-span-3 hidden lg:block sticky top-24 h-fit space-y-10">
                                    ...
                                    <nav className="space-y-2 border-l-2 border-slate-100 pl-4">
                                        {toc.map((item) => (
                                            <a
                                                key={item.id}
                                                href={`#${item.id}`}
                                                className={`block text-sm transition-all hover:text-primary ${item.level === 3 ? "pl-4 text-slate-500" : "font-medium text-slate-700"
                                                    }`}
                                            >
                                                {item.text}
                                            </a>
                            ...
                            <div className="space-y-4 pt-10 border-t border-slate-100">
                                ...
                                <div className="flex flex-col gap-2">
                                    <Button variant="outline" size="sm" className="justify-start hover:bg-[#1877F2] hover:text-white transition-colors">
                                        <Facebook className="mr-2 h-4 w-4" /> Facebook
                                    </Button>
                                    <Button variant="outline" size="sm" className="justify-start hover:bg-[#1DA1F2] hover:text-white transition-colors">
                                        <TwitterIcon className="mr-2 h-4 w-4" /> Twitter
                                    </Button>
                                    <Button variant="outline" size="sm" className="justify-start hover:bg-[#0A66C2] hover:text-white transition-colors">
                        ...
                        <article className="lg:col-span-6 space-y-12">
                            <div
                                ...
                                className="prose prose-lg md:prose-xl prose-slate max-w-none 
                                    ...
                                    prose-blockquote:border-primary prose-blockquote:bg-slate-50 prose-blockquote:py-1 prose-blockquote:px-6 prose-blockquote:rounded-r-2xl prose-blockquote:font-serif prose-blockquote:italic"
                                dangerouslySetInnerHTML={{ __html: processedBody }}
                            />

                            {/* Author Bio Section */}
                            <div className="bg-slate-50 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row gap-8 items-center border border-slate-100">
                                <div className="w-24 h-24 rounded-full bg-primary/20 flex items-center justify-center text-4xl text-primary font-black shrink-0 border-4 border-white shadow-lg">
                                    {post.author_name?.[0]?.toUpperCase()}
                                </div>
                                <div className="text-center md:text-left space-y-4">
                                    <div className="space-y-1">
                                        <span className="text-xs font-bold text-primary uppercase tracking-widest">Qoraaga</span>
                                        <h4 className="text-2xl font-bold text-slate-900">{post.author_name}</h4>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap justify-center md:justify-start gap-4">
                                        <Button variant="ghost" size="sm" className="rounded-full text-xs font-bold hover:text-primary">
                        ...
                        <aside className="lg:col-span-3 space-y-8">
                            <div className="bg-slate-900 rounded-3xl p-6 text-white overflow-hidden relative shadow-2xl">
                                <div className="relative z-10 space-y-4">
                                    <h3 className="font-bold text-xl leading-tight">Ku biir liiska aqristayaasha</h3>
                                    <p className="text-slate-400 text-sm">Hel ogeysiisyada qoraalada cusub isla marka la daabaco.</p>
                                    <div className="space-y-3">
                                        <input
                                            type="email"
                                            placeholder="Email-kaaga..."
                                            className="w-full bg-white/10 border-white/20 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                                        />
                            ...
                            {relatedPosts.length > 0 && (
                                <div className="space-y-6">
                                    <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400">Qoraaladaan xiiseey</h3>
                                    <div className="space-y-6">
                                        {relatedPosts.map(p => (
                                            <Link key={p.id} href={`/blog/${p.slug}`} className="group block space-y-3">
                                                ...
                                                <h4 className="font-bold text-slate-800 leading-snug group-hover:text-primary transition-colors line-clamp-2">
                                                    {p.title}
                                                </h4>
