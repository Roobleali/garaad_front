...
export function BlogListClient({ initialPosts }: BlogListClientProps) {
    ...
    const renderPostCard = (post: BlogPost) => {
        ...
        return (
            <Card key={post.id} className="group overflow-hidden border border-slate-100 bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                <div className="relative aspect-[16/10] w-full overflow-hidden">
                    ...
                    {post.tags.length > 0 && (
                        <Badge className="absolute top-4 left-4 z-20 bg-white/90 backdrop-blur-sm text-slate-900 hover:bg-white border-none shadow-sm capitalize">
                            {post.tags[0].name}
                        </Badge>
                    )}
                </div>
                ...
                <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-4 text-[11px] font-medium uppercase tracking-wider text-slate-500">
                    ...
                    >
                        <h3 className="font-bold text-xl md:text-2xl leading-tight line-clamp-2 decoration-primary/30 group-hover:underline underline-offset-4">
                            {post.title}
                        </h3>
                    </Link>
    ...
    return (
        <>
            ...
            <section className="relative pt-20 pb-16 md:pt-32 md:pb-24 bg-white border-b border-slate-100">
                        ...
                        <div className="relative max-w-xl mx-auto group">
                            ...
                            <div className="relative flex items-center">
                                ...
                                <Input
                                    ...
                                    className="pl-12 pr-4 py-7 bg-white border-slate-200 rounded-2xl shadow-sm focus:ring-2 focus:ring-primary/20 transition-all text-lg"
                                    value={searchTerm}
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
                    <div className="space-y-1">
                        <h2 className="text-2xl font-bold text-slate-900">Qoraaladii Ugu Dambeeyay</h2>
                        <p className="text-sm text-slate-500">Waxaan helnay {filteredPosts.length} qoraal oo ku habboon.</p>
                    </div>

                    {allTags.length > 0 && (
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full md:w-auto">
                            <TabsList className="bg-white border border-slate-100 h-11 p-1 rounded-xl shadow-sm overflow-x-auto max-w-full justify-start whitespace-nowrap">
                                <TabsTrigger value="all" className="rounded-lg px-6 data-[state=active]:bg-primary data-[state=active]:text-white">Dhammaan</TabsTrigger>
                                {allTags.slice(0, 5).map(tag => (
                                    <TabsTrigger key={tag.slug} value={tag.slug} className="rounded-lg px-6 data-[state=active]:bg-primary data-[state=active]:text-white">
                                        {tag.name}
                                    </TabsTrigger>
                                ))}
                            </TabsList>
                ...
                {filteredPosts.length === 0 ? (
                    <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-slate-200 shadow-sm max-w-2xl mx-auto">
                        ...
                        <h3 className="text-xl font-bold text-slate-800 mb-2">Waxbo lama helin</h3>
